...
const schemaToString = (typeName, prefix, postfix, definition, obj) => {
  let str = "";

  Object.keys(obj).forEach((key) => {
    const newTypeName = `${prefix}${pascalCase(key)}${postfix}`;
    if (Array.isArray(obj[key])) {
      ...
      if (Array.isArray(firstElement) || isObject(firstElement)) {
        str += schemaToString(newTypeName, prefix, postfix, firstElement);
    ...
    if (isObject(obj[key])) {
      str += schemaToString(newTypeName, prefix, postfix, obj[key]);
      obj[key] = newTypeName;
    }
    return "";
  });
...
module.exports = {
  stringifySchema: (typeName, prefix, postfix, definition, obj) =>
    removeDuplicateTypes(
      schemaToString(typeName, prefix, postfix, definition, obj)
    ),
};
