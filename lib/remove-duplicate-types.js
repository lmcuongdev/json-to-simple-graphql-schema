...
const combineTypes = (str, definition) =>
  ` ${str}`
    .split(` ${definition} `)
    // clear empty lines
    .filter((i) => i)
    .reduce((accum, fragment, idx) => {
      const full = `${definition} ${fragment}`.trim();
...
const extractPossibleIdenticalTypes = (str, definition) =>
  str
    .split(`${definition} `)
    // clear empty lines
    .filter((i) => i)
...
const removeDuplicateTypes = (str, definition) => {
  const schemaStr = Object.values(combineTypes(str, definition))
    .sort((a0, a1) => a0.idx - a1.idx)
    .map(({ full }) => full)
    .join("\n\n");

  const probablyDuplicates = Object.values(
    extractPossibleIdenticalTypes(schemaStr, definition)
  )
